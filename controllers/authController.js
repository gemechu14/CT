...
exports.login = async (req, res, next) => {
  try {
    ...
    const tenantStatus = await Tenant.findOne({
      where: { id: Number(user.defaultTenant) },
    });
    ...
    if (!tenantStatus.isActive) {
      return next(
        createError.createError(
          400,
          "Your tenant is currently suspended, and you cannot log in"
        )
      );
    }
  ...
  try {
    ...
    if (userCount > 0) {
      return next(
        createError.createError(
          500,
          "You are already signed up by this package"
        )
      );
    }
    ...
    const newThemeBranding = await ThemeBranding.create(
      {
        logoImage:
          "https://cedarplatform.io:4400/uploads/imageUrl-1718787785930-313132666.jpg",
        siteFaviconImage:
          "https://cedarplatform.io:4400/uploads/imageUrl-1718787785930-313132666.jpg",
        customLoader:
          "https://cedarplatform.io:4400/uploads/customLoader-1720701314174-483869383.gif",
...
exports.logout = async (req, res) => {
  ...
  await User.update({ isLoggedIn: false }, { where: { id: req.user.id } });
...
exports.forgetPassword = async (req, res, next) => {
  try {
    ...
    if (!email) {
      return next(
        createError.createError(400, "Please provide an email address")
      );
    }
    ...
    if (!user) {
      return next(
        createError.createError(404, "No user found with this email")
      );
    }
    ...
    if (!tenantStatus.isActive) {
      return next(
        createError.createError(400, "Your tenant is currently suspended.")
      );
    }
...
exports.resetPassword = async (req, res, next) => {
  try {
    ...
    if (!user) {
      return next(
        createError.createError(400, "Token is invalid or has expired")
      );
    }
    ...
    if (!newPassword) {
      return next(
        createError.createError(400, "Please provide a new password")
      );
    }
