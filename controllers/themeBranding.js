...
exports.getCurrentThemeBranding = async (req, res, next) => {
    try {
      ...
      const themeBranding = await ThemeBranding.findAll({
        ...
        //   return {
        //     ...rest,
        //     logoImage: logoImage ? `http://35.84.123.119:4400/${logoImage}` : logoImage,
        //     siteFaviconImage: siteFaviconImage ? `http://35.84.123.119:4400/${siteFaviconImage}` : siteFaviconImage,
        //     customLoader: customLoader ? `http://35.84.123.119:4400/${customLoader}` : customLoader,
        //     secondaryLogoImage: secondaryLogoImage ? `http://35.84.123.119:4400/${secondaryLogoImage}` : secondaryLogoImage,
        //   };
        // });
        
        return res.status(200).json(themeBranding);
...
if(themeBranding){
    return next(createError.createError(400,"Branding already created update it"))
      ...
      const logoImagePath = req.files?.logoImage?.[0]?.path;
      const logoImage = logoImagePath ? `http://35.84.123.119:4400/${logoImagePath}` : null;
  
      const siteFaviconImagePath = req.files?.siteFaviconImage?.[0]?.path;
      const siteFaviconImage = siteFaviconImagePath ? `http://35.84.123.119:4400/${siteFaviconImagePath}` : null;
  
      const secondaryLogoImagePath = req.files?.secondaryLogoImage?.[0]?.path;
      const secondaryLogoImage = secondaryLogoImagePath ? `http://35.84.123.119:4400/${secondaryLogoImagePath}` : null;
  
      const customLoaderPath = req.files?.customLoader?.[0]?.path;
      const customLoader = customLoaderPath ? `http://35.84.123.119:4400/${customLoaderPath}` : null;
...
exports.updateThemeBranding = async (req, res, next) => {
  try {
    ...
    if (customLoader) {
      updates.customLoader = customLoader;
    }
